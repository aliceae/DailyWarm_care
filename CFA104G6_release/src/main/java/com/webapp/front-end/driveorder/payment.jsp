		...
		<%-- errorMessage --%>
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					...
					</select>月 <select class="form-control" id="expire_year" name="expire_year">
						<option value="" selected="selected">請選擇</option>
...
function check(arg){
	...
	for(let i=1;i<=12;i++){
		$("#expiryMonth").append(`<option value="${"${i}"}">${"${i}"}</option>`);
	}
	for(let i=2022;i<=2032;i++){
		$("#expire_year").append(`<option value="${"${i}"}">${"${i}"}</option>`);
	}	
</script>
</body>
</html>