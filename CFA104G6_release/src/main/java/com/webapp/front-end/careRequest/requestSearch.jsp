...
<div class="container"> 
		...
		<div class="row w-100">
		<div><h4>搜尋照護需求單 目前您的id為<%=id %></h4></div><br>
		<FORM METHOD="post"
...
<!-- 			<input type="submit" class="button" value="查詢"> -->
			  <button type="submit" class="button">查詢</button>
 			  <button type="reset" value="Reset" class="button">清除</button>
		</FORM>
		</div>
</div>
	</section>
    ...
    startTime.addEventListener('blur', () => {
    	if (startTime.value == 0 ) {
    		checkStartTime.innerHTML = '<span style="color:grey">未輸入開始日期則預設為今天</span>';
    	}
    ...
    endTime.addEventListener('blur', () => {
    	if (endTime.value == 0 ) {
    		checkEndTime.innerHTML = '<span style="color:grey">未輸入結束日期則預設搜尋未來所有需求單</span>';
    	}
